...
@router.get("")
def list_instances(esp_id: Optional[str] = None):
    with get_db() as conn:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        if esp_id:
            cur.execute("""SELECT si.*, t.model, b.brand_name, tr.name AS tree_name
                ...
                WHERE si.esp_id=%s ORDER BY si.id""", (esp_id,))
        else:
            cur.execute("""SELECT si.*, t.model, b.brand_name, tr.name AS tree_name
                FROM sensor_instances si
                JOIN sensor_templates t ON si.template_id=t.id
                JOIN sensor_brands b ON t.brand_id=b.id
                LEFT JOIN trees tr ON si.tree_id=tr.id ORDER BY si.id""")
        rows = cur.fetchall(); cur.close()
    return [dict(r) for r in rows]

@router.post("", status_code=201)
def create_instance(body: SensorInstanceCreate):
    try:
        with get_db() as conn:
            cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            cur.execute("INSERT INTO sensor_instances (esp_id,template_id,slave_address,custom_name) VALUES (%s,%s,%s,%s) RETURNING *",
                        (body.esp_id, body.template_id, body.slave_address, body.custom_name))
...
def update_instance(instance_id: int, body: SensorInstanceUpdate):
    with get_db() as conn:
        cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
